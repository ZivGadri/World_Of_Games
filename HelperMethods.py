...
def check_user_exist(user_name):
    scores_file_path = path.join(path.dirname(path.abspath(__file__)), "Resources", "scores.txt")

    if not path.exists(scores_file_path):
        with open(scores_file_path, 'a') as file_rw:
            file_rw.write(user_name.upper() + " : 0\n")
        return False
    else:
        ...
        with open(scores_file_path, 'r') as file_rw:
            ...
            if data:
                ...
                for data_item in data:
                    existing_user = str(data_item.split(" : ")[0].strip())
                    if user_name.upper() == existing_user.upper():
                        return True
        # User not found, append to file
        with open(scores_file_path, 'a') as file_rw:
            if fileHasContent:
                file_rw.write("\n" + user_name.upper() + " : 0\n")
            else:
